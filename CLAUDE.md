...
- **`scripts/build-all.mjs`** — regenerates entries, builds each deck to `<out>/<slug>/` at base `<prefix>/<slug>/` (absolute `--out`; Slidev resolves a relative `--out` against the entry dir), strips per-deck video copies (`videos/` + `videos-hq/`, served from the remote fallback), then emits the landing via **`scripts/gen-landing.mjs`**. Flags: `--out`, `--base <prefix>`, `--only a,b`, `--flat-base` (base `/` for QA, no landing; always includes drafts), `--include-drafts`, `--keep-videos`.
...
**Video source chain** (`VideoPlayer.vue`): `pnpm dev` tries `videos-hq/<src>` → `videos/<src>` → GitHub release; a production build tries the release first, then the two local dirs (only present in a `--keep-videos` build — the lecture-day offline fallback). Each `<source>` error advances the chain.
