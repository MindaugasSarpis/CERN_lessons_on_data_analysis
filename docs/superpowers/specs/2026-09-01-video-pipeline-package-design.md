...
Course-specific clips (stay in this course, release `videos-web`, same renaming rule): `vu_physics_faculty.mp4` (maintainer-supplied, the L01 reel opener; profile `standard`), `gtc_2020.mp4`, `technology_size_comparison.mp4`, `cpu_vs_gpu_demo.mp4`, `how_computer_memory_works.mp4` (`standard-tight`; will still exceed `max_size_mb` — accepted, warning only). Added 2026-09-06: `ff_zoom_master.mp4` (maintainer's 4K60 master, 4:42 — Google Earth pull-back from the Physics Faculty at Saulėtekis out to the cosmic web), the L01 cold open, encoded with the course manifest's first H.264 profile `web-h264` and published to the course `videos` release straight away rather than waiting for pass 2. Dropped: `VU_VM.mp4`, the orphan `VU_VM_Zoom.*` (a 2:31 HEVC cut of the same pull-back, superseded) and the misspelt duplicate `Perseverence_…` release asset (after pass 2; pass 1 still references it, §8.1.1).
