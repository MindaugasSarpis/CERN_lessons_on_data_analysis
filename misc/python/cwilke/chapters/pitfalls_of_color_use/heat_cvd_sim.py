"""Heat sequential scale under three CVD simulations (Wilke fig 19.5)."""
...
# Seven-step "Heat"-style ramp (dark red → light yellow)
HEAT = ["#8B0000", "#B32700", "#D55E00", "#E69F00",
        "#F0C659", "#F3DC8E", "#F7EFC0"]
...
def render() -> None:
    ...
    flat = axes.flatten()
    for ax, (title, kind) in zip(flat, PANELS):
        sim = [simulate(h, kind) for h in HEAT]
    ...
    save(fig, "pitfalls_of_color_use", "heat_cvd_sim")
