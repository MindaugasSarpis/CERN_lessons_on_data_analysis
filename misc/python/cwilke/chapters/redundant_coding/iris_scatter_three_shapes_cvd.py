"""CVD simulation of iris scatter with distinct shapes (Wilke fig 20.4)."""
...
def render() -> None:
    ...
    for ax, (title, kind) in zip(axes.flatten(), PANELS):
        for sp, (hex_col, marker) in SPECIES_STYLE.items():
            col = simulate(hex_col, kind)
    ...
    fig.tight_layout()
    save(fig, "redundant_coding", "iris_scatter_three_shapes_cvd")
