"""mpg cty vs displ — small jitter reveals overplotted points
(Wilke fig 18.3)."""
...
def render() -> None:
    ...
    x_amp = 0.01 * (df["displ"].max() - df["displ"].min())
    y_amp = 0.01 * (df["cty"].max() - df["cty"].min())
    ...
    open_axes(ax)
    save(fig, "overlapping_points", "mpg_cty_displ_jitter")
