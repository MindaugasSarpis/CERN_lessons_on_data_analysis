"""Three biomedical preprint servers — lines with dots (Wilke fig 13.6)."""
...
def render() -> None:
    ...
    for arch in ("bioRxiv", "arXiv q-bio", "PeerJ Preprints"):
        ...
        ax.plot(sub["date"], sub["count"], color=COLORS[arch], linewidth=1.4)
        ax.scatter(sub["date"], sub["count"], s=30,
    ...
    open_axes(ax)
    save(fig, "time_series", "bio_preprints_lines")
