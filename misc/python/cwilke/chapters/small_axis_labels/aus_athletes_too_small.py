"""Aus athletes with all text set unreadably small (Wilke fig 24.1 — bad)."""
...
from cwilke.theme import OKABE_ITO, apply_base, grid, save, stamp_bad


SPORTS = ["field", "water polo", "basketball", "swimming", "track"]


def _build_axes(fig, ax, df, *, font_sz, marker_sz):
...
def render() -> None:
    ...
    _build_axes(fig, ax, df, font_sz=6, marker_sz=14)
    stamp_bad(fig)
    fig.tight_layout()
    save(fig, "small_axis_labels", "aus_athletes_too_small")
