...
def format_fig(width = 16, height = 9):
    ...
    [ax.spines[i].set_linewidth(width/6) for i in ["top", "right", "left", "bottom"]]


    return fig, ax 

def plot_hist(data, n_bins = None, axis_padding = None, width=16, height=9, xlow=None, xup=None):
    ...
    if n_bins is None:
        n_bins = int(np.floor(xup) - np.floor(xlow) + 1)
